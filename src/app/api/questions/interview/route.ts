...
// GET endpoint to fetch Interview questions by topicId
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch all Interview questions for the topicId
    const questions = await InterviewQuestion.find({ topicId });
    ...
    return NextResponse.json(
      { error: "Failed to fetch Interview questions", details: error.message },
      { status: 500 }
    );
  }
}

// POST endpoint to save Interview questions
export async function POST(request: NextRequest) {
  try {
    ...
    for (const question of questions) {
      if (!question.question || !question.answer || !question.explanation) {
        return NextResponse.json(
          {
            error:
              "Invalid question format. Each question must have: question, answer, explanation",
          },
          { status: 400 }
        );
      }
    ...
    return NextResponse.json(
      {
        ...
        testsCreated,
        message: `Successfully saved ${savedCount} Interview questions${
    ...
    return NextResponse.json(
      { error: "Failed to save Interview questions", details: error.message },
      { status: 500 }
    );
  }
}
